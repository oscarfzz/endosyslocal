...
class PluginCampos(Plugin):
	...
	def _do_actualizar_elementos_campo(self, campo_id):
		...
		campo = registro_by_id(endotools.model.Campo, campo_id)
		if campo:
			...
			for k, v in self.campos_actualizables.iteritems():
				if campo.nombre.upper() in v:
					...
					for codigo, nombre in elementos.iteritems():
						...
						q = meta.Session.query(endotools.model.Elemento).filter(
										and_(*[
											(endotools.model.Elemento.codigo == codigo),
											(endotools.model.Elemento.campo_id == campo_id)
										])
							)
						if q.count() > 0:
							elemento = q.one()
							...
							elemento = endotools.model.Elemento()
					...
					q = meta.Session.query(endotools.model.Elemento).filter(endotools.model.Elemento.campo_id == campo_id)
					lista = q.all()
