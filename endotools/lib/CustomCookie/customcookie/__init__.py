...
from endotools.lib.usuarios.conexiones import desconectar_usuario

#
# Setting up logging
#

log = logging.getLogger('authkit.authenticate.customcookie')
...
class CustomCookieUserSetter(AuthKitUserSetter, AuthTKTMiddleware):
    ...
    def __init__(self, app, secret, name='authkit', params=None,
        ...
        log.debug("Setting up the customcookie middleware")
        secure = False
    ...
    def __call__(self, environ, start_response):
        ...
        if cookie_value:
            ...
            environ['AUTH_TYPE'] = 'customcookie'
        set_cookies = []
        ...
        def logout_user():
            ...
            desconectar_usuario(environ.get('REMOTE_USER'))
...
def load_cookie_config(
    ...
    if user_setter_params.has_key('signout'):
        raise AuthKitConfigError(
            'The authkit.customcookie.signout option should now be named signoutpath'
        )
...
def make_cookie_user_setter(
    ...
    app = CustomCookieUserSetter(app, **user_setter_params)
    return app

# Backwards compatibility
make_handler = make_cookie_user_setter
AuthKitCookieMiddleware = CustomCookieUserSetter
