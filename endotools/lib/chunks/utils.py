...
def try_join_chunk_and_set_disponible(archivo):
	...
	try:
		...
		id = endotools.lib.capturas._get_captura_by_uuid(Captura, chunks_uuid)	
		...
		ruta_capturas = endotools.lib.capturas._construir_ruta(ruta_capturas, None, chunk_header['exploracion_id'])
		...
		if not copy_error:
			...
			os.remove(origen)
			endotools.lib.capturas._set_disponible(Captura, chunks_uuid)
			endotools.lib.capturas._create_thumbnail( endotools.lib.capturas._archivo(id) )
