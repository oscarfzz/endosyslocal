...
from endotools.lib.misc import isint
from pylons import config
from endotools.model import meta
from datetime import datetime
import endotools.model.worklist
...
def get_worklist_v2(modality=None, codigo_centro=None, campos=[], filtros_template=[], template_custom_path=None):
	...
	if config_worklist.lib == 'DCMTK':
		import endotools.lib.pydcmtk.pydcmtk
		import endotools.lib.dicom.dcmtk
		...
		worklist = endotools.lib.dicom.dcmtk.obtenerWorkList_v2(config_worklist, campos, filtros_template, template)
		
	elif config_worklist.lib == 'DCM4CHE': # No implementado ----------
		import endotools.lib.dicom.dcm4che
...
def get_worklist(date=None, modality=None, codigo_centro=None):
	...
	if config_worklist.lib == 'DCMTK':
		import endotools.lib.pydcmtk.pydcmtk
		import endotools.lib.dicom.dcmtk
		worklist = endotools.lib.dicom.dcmtk.obtenerWorkList(
						...
						date,
						modality,
						(   endotools.lib.pydcmtk.pydcmtk.kAccessionNumber,
							endotools.lib.pydcmtk.pydcmtk.kPatientID,
							endotools.lib.pydcmtk.pydcmtk.kPatientName,
							endotools.lib.pydcmtk.pydcmtk.kPatientBirthDate,
							endotools.lib.pydcmtk.pydcmtk.kPatientSex,
	...
	##						kScheduledProcedureStepID,
							endotools.lib.pydcmtk.pydcmtk.kStudyInstanceUID),
						'plantilla_query_wl')
	elif config_worklist.lib == 'DCM4CHE':
		import endotools.lib.dicom.dcm4che
		worklist = endotools.lib.dicom.dcm4che.obtenerWorkList(
						config_worklist.callingAE,
						config_worklist.calledAE,
						config_worklist.server,
						config_worklist.port,
	...
	for work in worklist:
		...
		if r:
			...
			r = endotools.model.worklist.Work()
			r.date_queried = date
			r.cita_id = None
...
def update_work(workid, **kwargs):
	...
	"""
	if isinstance(workid, endotools.model.worklist.Work):
		w = workid
	else:
		w = endotools.model.worklist.get_work(accessionNumber=workid)
	if w:
...
def get_work(workid):
	...
	"""
	return endotools.model.worklist.get_work(accessionNumber=workid)
