...
def obtenerWorkList(callingAE, calledAE, server, port, fecha, modality, campos=None, nombre_plantilla=None):
	...
	##			dataset.datasetValues[kModality] = modality
			...
			#   y por fecha
			if fecha:
				if isinstance(fecha, date): fecha = endotools.lib.dicom.util.date_to_DICOM(fecha)	#   si la fecha es tipo datetime.date, parsear como str
				n = dataset.getSequenceItem(kScheduledProcedureStepSequence)
	...
	try:
		...
		try:
			...
			for campo in filtros:
				if isinstance(campo["valor"], date): campo["valor"] = endotools.lib.dicom.util.date_to_DICOM(campo["valor"])
