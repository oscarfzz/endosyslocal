...
from endotools.lib.fusiones import FusionarPacientes
...
from endotools.lib.valores import get_valor_class_by_tipo
from base64 import b64encode
from endotools.lib.hl7_wrapper.log import nuevo_log_HL7_output, cambio_estado_log_HL7
import endotools.lib.hl7_wrapper
import endotools.lib.hl7_wrapper.receiving
from endotools.lib.hl7_wrapper import HL7_ABSENT
...
from endotools.lib.elementos import get_by_id
from endotools.lib.informes.valores import get_no_definido_value
...
def _make_MSH(centro, message_code, trigger_event, message_structure):
    ...
    datetime_message =      endotools.lib.hl7_wrapper.datetime_to_hl7(datetime.datetime.now())
    message_control_id =    endotools.lib.hl7_wrapper.datetime_to_hl7(datetime.datetime.now(), True)
...
def _make_PID(centro, paciente):
    ...
    patient_ids = [
        endotools.lib.hl7_wrapper.CX(paciente.CIP,           "", "JHN"),
        endotools.lib.hl7_wrapper.CX(paciente.DNI,           "", "NNESP"),
        endotools.lib.hl7_wrapper.CX(paciente.idunico,       "", "PN"),
        endotools.lib.hl7_wrapper.CX(paciente.numAfiliacion, "", "SS")
    ]
    ...
    if len(rels) > 0:
        patient_ids.append( endotools.lib.hl7_wrapper.CX(rels[0].nhc, centro.codigo, "PI") )

    patient_id_list = endotools.lib.hl7_wrapper.FIELD_REPEAT_SEPARATOR.join(patient_ids)
    
    pid = "PID||%s|%s||%s^%s|%s|%s|%s|||%s^^%s^%s^%s\r" % (
                    endotools.lib.hl7_wrapper.to_str(paciente.idunico),
                    patient_id_list,
                    endotools.lib.hl7_wrapper.to_str(paciente.apellido1),
                    endotools.lib.hl7_wrapper.to_str(paciente.nombre),
                    endotools.lib.hl7_wrapper.to_str(paciente.apellido2),
                    endotools.lib.hl7_wrapper.date_to_hl7(paciente.fechaNacimiento),
                    endotools.lib.hl7_wrapper.to_sex(paciente.sexo),
                    endotools.lib.hl7_wrapper.to_str(paciente.direccion),
                    endotools.lib.hl7_wrapper.to_str(paciente.poblacion),
                    endotools.lib.hl7_wrapper.to_str(paciente.provincia),
                    endotools.lib.hl7_wrapper.to_str(paciente.codigoPostal)
                )
...
def _crear_mensaje_oru_r01(informe, versiones=False, incrustado=False, anular=False):
    ...
    if exploracion.cita.work:
        ...
        obr = obr % (exploracion.cita.work.placerOrderNumber or '',
                     exploracion.id or '',
                     obr_prestacion,
                     endotools.lib.hl7_wrapper.datetime_to_hl7(fechahora))
        ...
        obr[7] = endotools.lib.hl7_wrapper.datetime_to_hl7(fechahora)
        obr = unicode(obr) + '\r'
...
def consulta_pacientes(id_unico_hl7):
    ...
    if tipo_mensajeria == 'QBP_RSP':
        ...
        query_tag = endotools.lib.hl7_wrapper.datetime_to_hl7(datetime.datetime.now())
        qpd = qpd % (   query_tag,
                        id_unico_hl7,
                        tipo_identificador
                    )
        ...
        try:
            ...
            if response.status == 404:
                ...
                hl7_process = endotools.lib.hl7_wrapper.receiving.Hl7Process(data)
...
def consulta_citas(codigo_agenda, fecha):
    ...
    if tipo_mensajeria == 'SQM_SQR':
        ...
        param = endotools.lib.hl7_wrapper.datetime_to_hl7(datetime.datetime.now())
        qrd = qrd % (param, param, codigo_agenda)

        # QRF
        qrf = 'QRF||%s|%s\r'
        param = endotools.lib.hl7_wrapper.datetime_to_hl7(fecha)
        qrf = qrf % (param, param)
...
def enviar_captura_actividad(cita, estado, motivo_id=None):
    ...
    # if hasattr(cita, 'exploracion') and estado:
        ...
        fecha_expl = endotools.lib.hl7_wrapper.datetime_to_hl7(fechahora)  # Hasta la versión 2.3.3.4 se enviaba solo la fecha!
    ...
    if tipo_mensaje_order:
        ...
        if cita.work:
            obr_prestacion = (cita.work.schProcStepID or endotools.lib.hl7_wrapper.HL7_ABSENT) + '^' + (cita.work.schProcStepDescription or endotools.lib.hl7_wrapper.HL7_ABSENT)
            obr = 'OBR|1|%s|%s|%s|||%s|||||||||||||||||\r'
            ...
            obr_prestacion = (cita.ex.prestacion_cod or endotools.lib.hl7_wrapper.HL7_ABSENT) + '^' + (cita.ex.prestacion_descr or endotools.lib.hl7_wrapper.HL7_ABSENT)
            ...
            if campo_prestacion_realizada:
                ...
                try:
                    if cita.exploracion:
                        ...
                        obr_prestacion =    obr_prestacion + '^^' +\
                                            (elemento_prestacion_realizada.codigo or endotools.lib.hl7_wrapper.HL7_ABSENT) + '^' +\
                                            (elemento_prestacion_realizada.nombre or endotools.lib.hl7_wrapper.HL7_ABSENT)
        ...
        if cita.work:
            orc_motivo_cancel = endotools.lib.hl7_wrapper.HL7_ABSENT
