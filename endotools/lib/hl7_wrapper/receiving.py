...
import hl7
import datetime
import endotools.lib.pacientes
import endotools.lib.citas
from endotools.model import meta
import endotools.model.prioridades
from endotools.model.motivosCancelacion import MotivoCancelacion
from endotools.lib.fusiones import FusionarPacientes, FusionarPacientesM2
from endotools.model.servicios import get_servicio_id, get_servicio_y_agenda_id
from endotools.model.agendas import get_agenda_id, get_agenda
from endotools.model.campos import Campo
from endotools.model.valores import ValorSelec
from endotools.lib.misc import *
from sqlalchemy.sql import and_, or_
from endotools.config.plugins import pluginHL7
from endotools.lib.informes import get_pdf
from base64 import b64encode
from endotools.lib.hl7_wrapper.log import nuevo_log_HL7
import endotools.lib.hl7_wrapper
import endotools.lib.registro as registro
from endotools.model.provincias import provincia_nombre_by_codigo
from endotools.model.poblaciones import poblacion_nombre_by_codigo
from endotools.model.pacientes import Rel_Pacientes_Centros
from endotools.model.centros import get_centro_id
...
class Hl7Process:
    ...
    def procesar_paciente(self, crearlo=True, multiples_pacientes=False):
        ...
        for pid in pids:
            ...
            fecha_nacimiento =  endotools.lib.hl7_wrapper.hl7_to_date(pid.get(7.1), descartar_hora=True)
            ...
            def asignar_campos(paciente):
                ...
                if not endotools.lib.hl7_wrapper.hl7absent(id_unico):        paciente.idunico =          endotools.lib.hl7_wrapper.hl7val(id_unico)
                if not endotools.lib.hl7_wrapper.hl7absent(dni):             paciente.DNI =              endotools.lib.hl7_wrapper.hl7val(dni)
                if not endotools.lib.hl7_wrapper.hl7absent(cip):             paciente.CIP =              endotools.lib.hl7_wrapper.hl7val(cip)
                if not endotools.lib.hl7_wrapper.hl7absent(numss):           paciente.numAfiliacion =    endotools.lib.hl7_wrapper.hl7val(numss)
                #
                if type(paciente.numAfiliacion) in (str, unicode): paciente.numAfiliacion = paciente.numAfiliacion[:50]
                #
                if not endotools.lib.hl7_wrapper.hl7absent(nombre):          paciente.nombre =           endotools.lib.hl7_wrapper.hl7val(nombre)
                if not endotools.lib.hl7_wrapper.hl7absent(apellido1):       paciente.apellido1 =        endotools.lib.hl7_wrapper.hl7val(apellido1)
                if not endotools.lib.hl7_wrapper.hl7absent(apellido2):       paciente.apellido2 =        endotools.lib.hl7_wrapper.hl7val(apellido2)
                if not endotools.lib.hl7_wrapper.hl7absent(sexo):                paciente.sexo =             endotools.lib.hl7_wrapper.hl7val(sexo)
                if not endotools.lib.hl7_wrapper.hl7absent(fecha_nacimiento): paciente.fechaNacimiento =     endotools.lib.hl7_wrapper.hl7val(fecha_nacimiento)
                if not endotools.lib.hl7_wrapper.hl7absent(direccion):       paciente.direccion =        endotools.lib.hl7_wrapper.hl7val(direccion)

                if not endotools.lib.hl7_wrapper.hl7absent(poblacion):
                    if config.get('INTEGRACION.POBLACIONES', '0') == '1':
                        paciente.poblacion =        poblacion_nombre_by_codigo(endotools.lib.hl7_wrapper.hl7val(poblacion))
                    else:
                        paciente.poblacion =        endotools.lib.hl7_wrapper.hl7val(poblacion)

                if not endotools.lib.hl7_wrapper.hl7absent(provincia):
                    if config.get('INTEGRACION.PROVINCIAS', '0') == '1':
                        paciente.provincia =        provincia_nombre_by_codigo(endotools.lib.hl7_wrapper.hl7val(provincia))
                    else:
                        paciente.provincia =        endotools.lib.hl7_wrapper.hl7val(provincia)

                if not endotools.lib.hl7_wrapper.hl7absent(codigoPostal):        paciente.codigoPostal =     endotools.lib.hl7_wrapper.hl7val(codigoPostal)
                if not endotools.lib.hl7_wrapper.hl7absent(telefono1):       paciente.telefono1 =        endotools.lib.hl7_wrapper.hl7val(telefono1)
                if not endotools.lib.hl7_wrapper.hl7absent(telefono2):       paciente.telefono2 =        endotools.lib.hl7_wrapper.hl7val(telefono2)
                if not endotools.lib.hl7_wrapper.hl7absent(numero_expediente): paciente.numero_expediente = endotools.lib.hl7_wrapper.hl7val(numero_expediente)
            ...
            paciente = endotools.lib.pacientes.get_by_idunico(id_unico)
    ...
    ##               paciente_updating.__setattr__(campo,paciente.__getattribute__(campo))
            ...
            if not paciente:
                paciente = endotools.lib.pacientes.nuevo_paciente()
            ...
            endotools.lib.pacientes.guardar_paciente(paciente, self)
    ...
    def fusionar_paciente(self):
        ...
        if modo == '0':
            ...
            paciente_origen = endotools.lib.pacientes.get_by_idunico(id_unico_origen)
            ...
            # Obtener NHC origen
            d = endotools.lib.hl7_wrapper.get_patient_identifier_list(self.msg.segment('MRG')[1])
            nhc_origen = endotools.lib.hl7_wrapper.get_nhc_centro(d, codigo_centro)
            idunico_origen = endotools.lib.hl7_wrapper.get_idunico(d)
            if nhc_origen == None:
            ...
            # Obtener NHC destino
            d = endotools.lib.hl7_wrapper.get_patient_identifier_list(self.msg.segment('PID')[3])
            nhc_destino = endotools.lib.hl7_wrapper.get_nhc_centro(d, codigo_centro)
            idunico_destino = endotools.lib.hl7_wrapper.get_idunico(d)
            if nhc_destino == None:
    ...
    def _obtener_agenda_id(self, codigo_agenda=None, codigo_servicio=None):
        ...
        if modo == 'AGENDA':
            if not endotools.lib.hl7_wrapper.hl7absent(codigo_agenda):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(codigo_servicio):
    ...
    def modificar_cita_ORM(self, paciente_id):
        ...
        fecha_hora =                endotools.lib.hl7_wrapper.hl7val(obr.get(7.1))                  #   HL7: Observation Date/Time
        codigo_prestacion =         endotools.lib.hl7_wrapper.hl7val(obr.get(4.1))                  #   HL7: Universal Service ID (identifier)
        descr_prestacion =          endotools.lib.hl7_wrapper.hl7val(obr.get(4.2))                  #   HL7: Universal Service ID (text)
        codigo_agenda =             endotools.lib.hl7_wrapper.hl7val(obr.get(21.1))                 #   HL7: Filler Field (Candelaria: código de agenda)
        
        
        numero_peticion =           endotools.lib.hl7_wrapper.hl7val(orc.get(2.1))                  #   HL7: Placer Order Number
        codigo_prioridad =          endotools.lib.hl7_wrapper.hl7val(orc.get(7.6))                  #   HL7: Priority (implementado para candelaria. Valores( 1-normal, 2-preferente, 3-urgente )
        medico_peticionario_cod =   endotools.lib.hl7_wrapper.hl7val(orc.get(19.1))                 #   HL7: Action By (ID Number)
        medico_peticionario_descr = '%s, %s' % (orc.get(19.2), orc.get(19.3))               #   HL7: Action By (Family Name, Given Name)    XXX falta hl7val()

        codigo_servicio =           endotools.lib.hl7_wrapper.hl7val(pv1.get(10.1))                 #   HL7: Hospital Service
        numero_episodio =           endotools.lib.hl7_wrapper.hl7val(pv1.get(19.1))				    #   HL7: Visit Number
        
        observaciones =             endotools.lib.hl7_wrapper.hl7val(nte.get(3.1)) if nte else None #   HL7: Comment
        ...
        def asignar_campos(cita):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(fecha_hora):
                cita.fecha =                endotools.lib.hl7_wrapper.hl7_to_date(fecha_hora, descartar_hora=True)
                cita.hora =                 endotools.lib.hl7_wrapper.hl7_to_datetime(fecha_hora)
                
            if not endotools.lib.hl7_wrapper.hl7absent(codigo_prioridad):
                cita.prioridad_id =         endotools.model.prioridades.prioridad_id_by_codigo(codigo_prioridad)
                
            if not endotools.lib.hl7_wrapper.hl7absent(observaciones):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(id_unico):
                cita.ex.idunico =           id_unico
            if not endotools.lib.hl7_wrapper.hl7absent(cip):
                cita.ex.cip =               cip
                
                
            if not endotools.lib.hl7_wrapper.hl7absent(numero_episodio):
                cita.ex.numero_episodio =   numero_episodio
            if not endotools.lib.hl7_wrapper.hl7absent(codigo_prestacion):
                cita.ex.prestacion_cod =    codigo_prestacion
            if not endotools.lib.hl7_wrapper.hl7absent(descr_prestacion):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(medico_peticionario_cod):
                cita.ex.medico_peticionario_cod = medico_peticionario_cod
            if not endotools.lib.hl7_wrapper.hl7absent(medico_peticionario_descr):
        ...
        cita = endotools.lib.citas.get_by_id_ext(numero_peticion, 'numero_peticion')
        if cita:
            ...
            # Modificar la cita
            asignar_campos(cita)
            endotools.lib.citas.guardar_cita(cita, "sysadmin", self.ipaddress, cita_updating, self.id_hl7_log)
        else:
    ...
    ##  descr_agenda = ...OBR.21.2¿?            
        ...
        def asignar_campos(cita):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(cip):
                cita.ex.cip =               cip
            if not endotools.lib.hl7_wrapper.hl7absent(numero_peticion):
        ...
        if cita:
            ...
            cita = endotools.lib.citas.nueva_cita()
            asignar_campos(cita)
            endotools.lib.citas.guardar_cita(cita, "sysadmin", self.ipaddress, None, self.id_hl7_log)
    ...
    def cancelar_cita_ORM(self):
        ...
        cita = endotools.lib.citas.get_by_id_ext(numero_peticion, 'numero_peticion') # XXX en este caso es el numero_peticion, pero en otras integraciones puede ser otro...
        if cita:
            #   existe, eliminarla
            endotools.lib.citas.eliminar_cita(cita, "sysadmin", self.ipaddress, self.id_hl7_log)
        else:
            #   no existe, error...
            pass
    ...
    def nueva_cita_SIU(self, paciente_id):
        ...
        fecha_hora =                endotools.lib.hl7_wrapper.hl7val(ais.get(4.1))
        ...
        codigo_prestacion =         endotools.lib.hl7_wrapper.hl7val(ais.get(3.1))   #   HL7: Universal Service ID (identifier)
        descr_prestacion =          endotools.lib.hl7_wrapper.hl7val(ais.get(3.2))   #   HL7: Universal Service ID (text)
        codigo_servicio =           endotools.lib.hl7_wrapper.hl7val(pv1.get(10.1))   #   HL7: Hospital Service
        numero_episodio =           endotools.lib.hl7_wrapper.hl7val(pv1.get(19.1))   #   HL7: Visit Number
        medico_peticionario_cod =   endotools.lib.hl7_wrapper.hl7val(pv1.get(8.1))   #   HL7: Referring Doctor
        medico_peticionario_descr = '%s, %s' % (pv1.get(8.2), pv1.get(8.3)) #   XXX falta hl7val()

        observaciones = None
        if nte:
            observaciones =             endotools.lib.hl7_wrapper.hl7val(nte.get(3.1))   #   HL7: Comment

        numero_cita =               endotools.lib.hl7_wrapper.hl7val(sch.get(1.1))   #   HL7: Placer Appointment ID
        codigo_agenda =             endotools.lib.hl7_wrapper.hl7val(sch.get(5.1))   #   HL7: Schedule ID (identifier)
        descr_agenda =              endotools.lib.hl7_wrapper.hl7val(sch.get(5.2))   #   HL7: Schedule ID (text)
        numero_peticion =           endotools.lib.hl7_wrapper.hl7val(sch.get(26.1))  #   Número de pedido/peticion asociado a la cita
        codigo_prioridad =          endotools.lib.hl7_wrapper.hl7val(sch.get(11.6))         # prioridad de la cita
        filler_status_code =        endotools.lib.hl7_wrapper.hl7val(sch.get(25.1))         # filler status code (booked, etc...)
        servicio_peticionario =     pv1.get(8.9).split("&")
        servicio_peticionario_cod = endotools.lib.hl7_wrapper.hl7val(servicio_peticionario[0])
        servicio_peticionario_descr =   None
        if  len(servicio_peticionario) > 1:
            servicio_peticionario_descr = endotools.lib.hl7_wrapper.hl7val(servicio_peticionario[1])
        ...
        def asignar_campos(cita):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(observaciones) and observaciones is not None:
                cita.observaciones =    observaciones
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(numero_peticion):
                cita.ex.numero_peticion =           numero_peticion
            if not endotools.lib.hl7_wrapper.hl7absent(numero_cita):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(codigo_agenda):
                cita.ex.agenda_cod =                codigo_agenda
            if not endotools.lib.hl7_wrapper.hl7absent(descr_agenda):
            ...
            if not endotools.lib.hl7_wrapper.hl7absent(medico_peticionario_descr):
                cita.ex.medico_peticionario_descr = medico_peticionario_descr
            if not endotools.lib.hl7_wrapper.hl7absent(servicio_peticionario_cod):
                cita.ex.servicio_peticionario_cod = servicio_peticionario_cod
            if not endotools.lib.hl7_wrapper.hl7absent(servicio_peticionario_descr):
                cita.ex.servicio_peticionario_descr = servicio_peticionario_descr
            if not endotools.lib.hl7_wrapper.hl7absent(filler_status_code):
    ...
    ##      citas.guardar_cita(cita)
        ...
        cita = endotools.lib.citas.get_by_id_ext(numero_cita, 'numero_cita')
        if not cita:
            cita = endotools.lib.citas.get_by_id_ext(numero_peticion, 'numero_peticion')
        #    si no existia la cita, la crea.
        if not cita: cita = endotools.lib.citas.nueva_cita()
    ...
    ##      cita.tipoExploracion_id =   get_tipoExploracion_id_from_prestacion(prestacion=codigo_prestacion, servicio=codigo_servicio)
        ...
        #   comprobar si ya existe, por ID EXT (no debería)
        cita = endotools.lib.citas.get_by_id_ext(numero_cita, 'numero_cita') # XXX en este caso es el numero_cita, pero en otras integraciones puede ser otro...
        if cita:
            #   existe, actualizarla
    ...
    def nuevas_citas_SQR(self, pacientes_ids):
        ...
        for idx, sch in enumerate(schs):
            ...
            fecha_hora =                endotools.lib.hl7_wrapper.hl7val(ais.get(4.1))  #   XXX VACIO
            codigo_prestacion =         endotools.lib.hl7_wrapper.hl7val(ais.get(3.1))  #   HL7: Universal Service ID (identifier)
            descr_prestacion =          endotools.lib.hl7_wrapper.hl7val(ais.get(3.2))  #   HL7: Universal Service ID (text)
        
            codigo_servicio =           endotools.lib.hl7_wrapper.hl7val(pv1.get(10.1))   #   HL7: Hospital Service XXX VACIO
            numero_episodio =           endotools.lib.hl7_wrapper.hl7val(pv1.get(19.1))   #   HL7: Visit Number (NUMICU)
            medico_peticionario_cod =   endotools.lib.hl7_wrapper.hl7val(pv1.get(8.1))   #   HL7: Referring Doctor XXX VACIO
            ...
            numero_cita =               endotools.lib.hl7_wrapper.hl7val(sch.get(2.1))   #   HL7: Filler Appointment ID
            ...
            codigo_agenda =             endotools.lib.hl7_wrapper.hl7val(ail.get(3.2))  #   HL7: Location Resource ID
            descr_agenda =              endotools.lib.hl7_wrapper.hl7val(endotools.lib.hl7_wrapper.HL7_ABSENT)    #   XXX NO VIENE
            
            numero_peticion =           endotools.lib.hl7_wrapper.hl7val(endotools.lib.hl7_wrapper.HL7_ABSENT)    #   XXX NO VIENE
            codigo_prioridad =          endotools.lib.hl7_wrapper.hl7val(endotools.lib.hl7_wrapper.HL7_ABSENT)    #   XXX NO VIENE
            filler_status_code =        endotools.lib.hl7_wrapper.hl7val(endotools.lib.hl7_wrapper.HL7_ABSENT)    #   XXX NO VIENE

            # No viene...
            servicio_peticionario_cod = endotools.lib.hl7_wrapper.hl7val(endotools.lib.hl7_wrapper.HL7_ABSENT)    #   XXX NO VIENE
            servicio_peticionario_descr =endotools.lib.hl7_wrapper.hl7val(endotools.lib.hl7_wrapper.HL7_ABSENT)   #   XXX NO VIENE
            ...
            cita = endotools.lib.citas.get_by_id_ext(numero_cita, 'numero_cita')
            if not cita:
                cita = endotools.lib.citas.get_by_id_ext(numero_peticion, 'numero_peticion') # XXX: de momento nunca vendra nº peticion, siempre nº cita
            #    si no existia la cita, la crea.
            if not cita: cita = endotools.lib.citas.nueva_cita()
            #   si la cita existe recuperamos la cita antes de ser modificada par poder registrarla
            cita_updating = None
