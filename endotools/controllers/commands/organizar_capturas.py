...
from endotools.lib.capturas import _mover_capturas_ruta_correcta
from endotools.lib.tareas import nueva_tarea, crear_hilo

log = logging.getLogger(__name__)


class OrganizarCapturasController(CommandController):

	#file_path = os.path.join(config['pylons.paths']['custom'], 'csv\import_pacientes.csv')

	def run_command(self):
		
		tarea = nueva_tarea("sysadmin",'ORG_CAP')
		#crea el hilo para realizar la tarea
		crear_hilo(tarea,{})

	def get(self, request):
		a_volver = u'<a href="/admin"> << Volver </a> <br>'
		h1 = u'<h1>Reorganización de capturas</h1>'
		descripcion = u'Mueve las capturas que se ubican en la raiz de la carpeta de capturas a su estructura correcta con el formato /año/mes'
