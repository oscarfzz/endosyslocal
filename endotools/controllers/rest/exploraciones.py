...
from endotools.model.motivosCancelacion import motivo_by_id
from endotools.model import Paciente
...
from endotools.config.plugins import pluginExploraciones
from endotools.lib.plugins.base.exploraciones import Exploracion_DICOM_data
import endotools.lib.dicom
import endotools.lib.dicom.util
from endotools.lib.formularios import nuevo_valor
from endotools.lib.busquedas import ejecutar_busqueda
import endotools.lib.registro as registro
import endotools.lib.organizacion_centros as organizacion_centros
from endotools.lib.misc import *
from endotools.lib.exploraciones import *
from sqlalchemy.sql import or_
from pylons import config
from endotools.lib.dicom.util import create_series_instance_uid
...
##              dicom_data.studyTime = endotools.lib.dicom.util.time_to_DICOM(exploracion.hora)
        ...
        if dicom_data:
            ...
            if dicom_data.patientBirthDate == None:
                ed.patientBirthDate = endotools.lib.dicom.util.date_to_DICOM(exploracion.paciente.fechaNacimiento)
            elif isinstance(dicom_data.patientBirthDate, datetime.date):
                ed.patientBirthDate = endotools.lib.dicom.util.date_to_DICOM(dicom_data.patientBirthDate)
            else:
            ...
            if dicom_data.studyDate == None:
                ed.studyDate = endotools.lib.dicom.util.date_to_DICOM(exploracion.fecha)
            elif isinstance(dicom_data.studyDate, datetime.date):
                ed.studyDate = endotools.lib.dicom.util.date_to_DICOM(dicom_data.studyDate)
            else:
                ed.studyDate = dicom_data.studyDate
            if dicom_data.studyTime == None:
                ed.studyTime = endotools.lib.dicom.util.time_to_DICOM(exploracion.hora)
            elif isinstance(dicom_data.studyDate, datetime.time):
                ed.studyTime = endotools.lib.dicom.util.time_to_DICOM(dicom_data.studyTime)
            else:
...
##          endotools.lib.dicom.PACS.store(exploracion.id)
        ...
        if (config.get('ENVIAR_CAPTURA_ACTIVIDAD.ACTIVO', '0') == '1'):
            if exploracion.cita:
                ...
                endotools.lib.hl7_wrapper.sending.enviar_captura_actividad(exploracion.cita, True)
    ...
    def _cancela_exploracion(self, exploracion, motivo_id):
        ...
        if config.get('ENVIAR_CAPTURA_ACTIVIDAD.ACTIVO', '0') == '1':
            if exploracion.cita:
                import endotools.lib.hl7_wrapper.sending
                endotools.lib.hl7_wrapper.sending.enviar_captura_actividad(exploracion.cita, False, motivo_id)
