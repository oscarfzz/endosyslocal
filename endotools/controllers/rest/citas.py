...
from endotools.model.citas_ex import get_cita_ex
...
from endotools.lib.citas import nueva_cita, guardar_cita, from_params, get_hora_fin, CAMPOS_CITA_EX, set_cita
from endotools.lib.plugins.base import *
from endotools.config.plugins import pluginCitas
import paste.httpexceptions as httpexceptions
import endotools.lib.hl7_wrapper.sending as hl7_sending
import endotools.lib.misc
import endotools.model.agendas
from endotools.model import Configuracion
from endotools.lib.pydicom_wrapper.mwl import WorklistRequest
from endotools.lib.dicom.worklist.utils import convert_keys_str2hex
from endotools.lib.dicom.worklist.receiving import WorklistProcess
    ...
    def _doIndex(self, params, format='xml'):
        ...
        if not pluginCitas or (not self.fecha):
            ...
            if (config.get('HL7.CONSULTA_CITAS.ACTIVO', '0') == '1') and \
                ...
                agenda = endotools.lib.misc.registro_by_id(endotools.model.agendas.Agenda, self.agenda_id)
                ...
                obj_nivel = {}
                obj_nivel['3'] = endotools.lib.misc.registro_by_id(endotools.model.agendas.Agenda, self.agenda_id)
...
##          p['medico_id'] = medico.id
        ...
        if 'agenda_id' in p:
            ...
            self.agenda_id = None
        endotools.lib.organizacion_centros.set_default(username, ipaddress, agenda = self.agenda_id)
...
##              abort_xml(500, 'Ha ocurrido un error cargando la cita (%s)' % e)
    ...
    def update(self, id):
        ...
        from endotools.lib.plugins.base.citas import PluginCitas
...
##          medico = medico_from_user(request.environ['REMOTE_USER'])
        ...
        if config.get('ENVIAR_CAPTURA_ACTIVIDAD.ACTIVO', '0') == '1':
            ...
            endotools.lib.hl7_wrapper.sending.enviar_captura_actividad(cita, False, motivo_id)
