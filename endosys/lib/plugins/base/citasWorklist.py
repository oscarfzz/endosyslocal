...
##			self.from_work(work)
	...
	def from_work(self, work):
		...
		self.hora = endosys.lib.dicom.util.DICOM_to_datetime(work.schProcStepStartDate, work.schProcStepStartTime)	#   hora de la cita
		...
		self.paciente.fechaNacimiento = endosys.lib.dicom.util.DICOM_to_date(work.patientBirthDate)
...
##		tipoexploracion_id = getTipoExploracionId(cita_de_ws.CodPrestacion)
		...
		if (tipoexploracion_id):
			tipoexploracion = registro_by_id(endosys.model.tiposExploracion.TipoExploracion, tipoexploracion_id)
...
class PluginCitasWorklist(PluginCitas):
	...
	def index(self, params):
		...
		if 'agenda_id' in params:
			agenda = registro_by_id(endosys.model.Agenda, params['agenda_id'])
		...
		worklist = endosys.lib.dicom.get_worklist(fecha, None, codigo_centro)
...
##			if not pluginPacientes:
			...
			if q.count() > 0:
				...
				paciente = endosys.model.pacientes.Paciente()
...
##				paciente.fechaNacimiento = cita.paciente.fechaNacimiento
			...
			if not awork.cita_id:
				...
				awork = endosys.lib.dicom.update_work(awork, cita_id = nuevaCita.id)
...
##				cita.tipoExploracion.id = cita.tipoExploracion_id
				registroCita = registro_by_id(endosys.model.citas.Cita, awork.cita_id)
	...
	def show(self, id):
		...
		cita = registro_by_id(endosys.model.citas.Cita, id)
		if cita is None:
	...
	def nueva_cita_BBDD(self, cita):
		...
		nuevaCita = endosys.model.citas.Cita()
		for campo in endosys.model.citas.Cita.c.keys():
			if campo == 'id': continue
			if not campo in cita.__dict__: continue
			if not isinstance(getattr(endosys.model.citas.Cita, campo), InstrumentedAttribute): continue
